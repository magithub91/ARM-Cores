...
static const unsigned char width = 128;
static const unsigned char height = 64;

static unsigned char CAddress = 0x78;
static unsigned char DAddress = 0x7A; //0x79
...
static bool oled_frame[width][height];	//kept public because of big size, check for alternative

void SSD1306_init(I2C_HandleTypeDef *hi2c1);
void drawFullscreen(I2C_HandleTypeDef *hi2c1);	//I2C_HandleTypeDef *hi2c1
void drawFullscreen2(bool newFrame[128][64]);	//I2C_HandleTypeDef *hi2c1
void setContrast(unsigned char contrast);
void inverseDisplay(bool isInverse);
unsigned char getCAddress(void);
unsigned char getDAddress(void);
void pixel(int x0, int y0, bool colour, bool locked );	//if locked won't plot outside of region
void pixels3V(int  x0, int y0, bool colour, bool locked );
void pixels3H(int x0, int y0, bool colour, bool locked );
void line(int x0, int y0, int x1, int y1, bool colour, bool locked );	//0 & 1 in last para. can be ambigiuous	//decide on exact int type
void line2(int x0, int y0, int x1, int y1, bool colour, int lineWidth);	//might just combine w/ the above
...
void rectangle2(int x0, int y0, int x1, int y1, bool colour, int lineWidth);	//works on any quad polygon really
...
void fill2(int x0, int y0, int x1, int y1, bool colour);	//fill defined region with one colour; same as drawing a filled rectangle; add error handling
void invert(void);	//same as inverseDisplay(), but done on frame array so probably less efficient
void invert2(int x0, int y0, int x1, int y1);	//a specific part of the screen
